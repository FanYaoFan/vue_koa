...
router.get('/insertAllGoodsInfo', async (ctx) =>{ 
    fs.readFile('./goods/goods.json','utf8', (err,data) => {
         let saveCount=0
...
router.get('/insertAllCategory' , async (ctx) => {
    // 第一步读取 category.json文件
    fs.readFile( './goods/category.json', 'utf8', (err,data) => {
        data = JSON.parse(data)
        // 引入 Category Schema 的模型
    ...
    ctx .body = "开始导入数据(category)"
})

// 子类  
router.get('/insertCategorySub', async (ctx) => {
    fs.readFile('./goods/category_sub.json', 'utf8', (err,data)=>{
        data = JSON.parse(data)
         // 得到映射对象
...
// 根据商品类别获取商品列表
router.post('/getGoodsSubID', async (ctx) => {
    
    try{
      // 分页的制作
        let categorySubId = ctx.request.body.categorySubId // 得到右侧小类数据的id
